...
const parseEmailBody = (html: string): { heading: string; description: string; buttonText: string } => {
  ...
  try {
    ...
    const buttonText = buttonElement ? buttonElement.textContent?.trim() || buttonElement.innerHTML.trim() : "";
...
const updateTextNodePreservingStructure = (element: Element, newText: string, doc: Document): void => {
  if (!element) return;
...
const replaceContentPreservingOuterStyles = (element: Element, newHtml: string, doc: Document): void => {
  if (!element) return;
  
  // Check if element has direct child spans with font-size styles
  const directChildren = Array.from(element.children);
  const styledSpans = directChildren.filter(
    (child) => child.tagName === 'SPAN' && child.getAttribute('style')?.includes('font-size')
  );
  ...
  if (styledSpans.length > 0) {
    ...
    for (const child of directChildren) {
      const nestedSpans = Array.from(child.children).filter(
        (nested) => nested.tagName === 'SPAN' && nested.getAttribute('style')?.includes('font-size')
      );
...
const reconstructEmailBody = (heading: string, description: string, buttonText: string, originalBody: string): string => {
  if (!originalBody) {
  ...
  try {
    ...
    let buttonElement = wrapper.querySelector('#u_content_button_1 a span span');
    if (!buttonElement) {
    ...
    if (buttonElement && buttonText) {
      ...
      if (buttonElement.tagName === 'SPAN') {
        ...
        const innerSpan = buttonElement.querySelector('span span') || buttonElement.querySelector('span');
        if (innerSpan) {
...
const EmailBodyEditor = ({ 
  ...
  const [activeBlock, setActiveBlock] = useState<'heading' | 'description' | 'button' | null>(null);
  ...
  const insertVariable = (variable: string, blockType: 'heading' | 'description') => {
    const quillRef = blockType === 'heading' ? headingQuillRef : descriptionQuillRef;
    if (quillRef.current) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div 
        ...
        style={{ padding: '30px 20px', minHeight: '100px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      >
              ...
              <div className="flex flex-col gap-4">
                ...
                {isEditMode ? (
                  <div className="flex flex-col gap-4">
                    ...
                    {/* Email Heading Input */}
                    <div className="flex flex-col gap-1">
                      <label className="text-sm text-[#303030]">Email Heading</label>
                      <input
                        type="text"
                        value={emailHeading}
                        onChange={(e) => {
                    ...
                    <div className="flex flex-col gap-2">
                      <label className="text-sm text-[#303030]">Banner Image</label>
                      ...
                      {(bannerImagePreview || originalImageUrl || selectedTemplate?.imageUrl) && (
                        <div className="mt-2">
                          <p className="text-xs text-gray-500 mb-2">Current banner image</p>
                          <div className="w-32 h-32 border border-gray-200 rounded-md overflow-hidden bg-white">
                            <img
                              src={bannerImagePreview || originalImageUrl || selectedTemplate?.imageUrl || ''}
                              alt="Banner preview"
                    ...
                    <div className="flex flex-col gap-2">
                      <label className="text-sm text-[#303030]">Email Body</label>
                      {typeof window !== 'undefined' && (
                        <EmailBodyEditor
                          ...
                          onHeadingChange={(value) => {
                            ...
                            const newBody = reconstructEmailBody(value, emailDescriptionText, emailButtonText, originalEmailBody);
                            setEmailBody(newBody);
                          }}
                          onDescriptionChange={(value) => {
                            setEmailDescriptionText(value);
                            // Reconstruct body for preview (updates only text nodes, preserves structure)
                            const newBody = reconstructEmailBody(emailHeadingText, value, emailButtonText, originalEmailBody);
                            setEmailBody(newBody);
                          }}
                          onButtonChange={(value) => {
                            setEmailButtonText(value);
                            // Reconstruct body for preview (updates only text nodes, preserves structure)
                            const newBody = reconstructEmailBody(emailHeadingText, emailDescriptionText, value, originalEmailBody);
                            setEmailBody(newBody);
                          }}
                ...
                {/* Email Preview */}
                <div className={`bg-white ${isEditMode ? 'h-auto pb-4' : 'h-[calc(100lvh-174px)]'} ${isEditMode ? '' : 'overflow-y-auto'}`}>
                  {loadingTemplate ? (
                    ...
                    (() => {
                      ...
                      if (selectedTemplate.imageUrl && !isEditMode) {
                        ...
                        const isFestivalTemplate = selectedTemplate.templateType === 'festival' || 
                                                   selectedEmail === 'festival' ||
                                                   selectedTemplate.name?.toLowerCase().includes('festival');
                        
                        const isCouponExpireTemplate = selectedTemplate.templateType === 'couponExpire' || 
                                                       selectedEmail === 'couponExpire' ||
                                                       selectedTemplate.name?.toLowerCase().includes('coupon') ||
                                                       selectedTemplate.heading?.toLowerCase().includes('coupon expiration');
                        
                        const needsSpecialStyling = isFestivalTemplate || isCouponExpireTemplate;
                        
                        let imageUrl = selectedTemplate.imageUrl;
                        
                        // If it's a Cloudinary URL and needs special styling, add white background transformation
                        if (needsSpecialStyling && imageUrl.includes('res.cloudinary.com')) {
                          // Add Cloudinary transformation to set white background
                          // Insert transformation before the filename
                          if (imageUrl.includes('/upload/')) {
                            imageUrl = imageUrl.replace('/upload/', '/upload/b_white/');
                          }
                        }
                        ...
                        if (needsSpecialStyling) {
                          ...
                          templateHtml = templateHtml.replace(/\{\{\{banner_image\}\}\}/g, bannerImageHtml);
                        } else {
                        ...
                        templateHtml = templateHtml.replace(/\{\{\{banner_image\}\}\}/g, '<div style="display: none;"></div>');
                      }
                      
                      // Replace heading placeholder if it exists
                      if (selectedTemplate.heading) {
                        templateHtml = templateHtml.replace(/\{\{\{heading\}\}\}/g, selectedTemplate.heading);
                      }
                      ...
                      if (isEditMode) {
                        ...
                        templateHtml = templateHtml.replace(/\{\{\{body_children\}\}\}/g, '<div style="display: none;"></div>');
                      } else {
                        // Use reconstructed body (with edited values) or fallback to original
                        const bodyContent = emailBody || originalEmailBody || selectedTemplate.body || "";
                        if (bodyContent) {
                          templateHtml = templateHtml.replace(/\{\{\{body_children\}\}\}/g, bodyContent);
                        }
                      }
                    ...
                    <div className="flex items-center justify-center py-20">
                      <div className="text-center text-gray-500">
                        ...
                        <p className="text-sm mt-2">Select an email from the list to preview</p>
                      </div>
                    </div>
                  )}
                </div>
