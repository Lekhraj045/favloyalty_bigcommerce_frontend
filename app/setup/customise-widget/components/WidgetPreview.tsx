...
import AnnouncementsSliderArea from "./AnnouncementsSlider";

// Helper function to get widget positioning styles based on widgetButton
function getWidgetPositionStyles(widgetButton: string) {
  const isRight =
    widgetButton === "bottom-right" || widgetButton === "top-right";
  const isTop = widgetButton === "top-left" || widgetButton === "top-right";

  return {
    justifyContent: isRight ? "flex-end" : "flex-start",
    alignSelf: isRight ? "flex-end" : "flex-start",
    order: isTop ? -1 : 1, // For top positioning, we'll use order to move it before the card
  };
}

export default function WidgetPreviewArea() {
  const { state } = useWidgetCustomization();
  const positionStyles = getWidgetPositionStyles(
    state.widgetButton || "bottom-right"
  );

  // Helper function to get pattern background style
  const getHeaderBackgroundStyle = () => {
    const baseStyle: React.CSSProperties = {
      backgroundColor: state.widgetBgColor || "#62a63f",
    };

    // If a pattern is selected (not "none"), add it as a background image overlay
    if (state.selectedPattern && state.selectedPattern !== "none") {
      const patternImage = `wizard-${state.selectedPattern}.svg`;
      baseStyle.backgroundImage = `url(${process.env.NEXT_PUBLIC_BASE_PATH}/images/${patternImage})`;
  ...
  return (
    <>
      <div className="flex flex-col gap-4 relative">
        <div
          className="w-full rounded-2xl border border-[#DEDEDE] bg-white shadow-sm overflow-hidden"
          style={{ order: 0 }}
        >
          {/* Header */}
          <div
          ...
          >
            <button className="absolute right-4 top-4 text-white text-medium cursor-pointer">
              <X className="w-4 h-4" />
            </button>
            ...
            <div className="flex items-center gap-2 mt-1">
              <span
                className="text-sm whitespace-nowrap overflow-hidden text-ellipsis max-w-[150px]"
                style={{ color: state.headingColor || "#ffffff" }}
              >
                John
              </span>
              <span
                className="text-xs bg-white px-2 py-0.5 rounded"
                style={{ color: state.widgetBgColor || "#62a63f" }}
              >
                Gold user 2x
              </span>
            </div>
            ...
            <div className="border border-[#DEDEDE] bg-white rounded-xl p-4">
              ...
              >
                <span className="text-xl">
                  <Image
                    src={`${process.env.NEXT_PUBLIC_BASE_PATH}/images/point-icon1.svg`}
                    alt="widget-icon1"
                    width={20}
                    height={20}
                    priority
            ...
            <div className="border border-[#DEDEDE] bg-white rounded-xl p-3 space-y-2">
                    ...
                    >
                      <g clipPath="url(#clip0_461_4754)">
                        <path
                          ...
                          fill={state.widgetBgColor || "#62a63f"}
                        ></path>
                        <path
              ...
              <div className="flex gap-2 mt-2">
                ...
                <button
                  ...
                  style={{
                    borderColor: state.widgetBgColor || "#62a63f",
                    color: state.widgetBgColor || "#62a63f",
                  }}
                >
                  Send
                </button>
        ...
        {state.selectedLauncher === "icon-only" && (
          <div
            className="flex"
            style={{
              justifyContent: positionStyles.justifyContent,
              order: positionStyles.order,
            }}
          >
            <div
              className="w-[60px] h-[60px] rounded-full flex items-center justify-center cursor-pointer"
              style={{ backgroundColor: state.widgetBgColor || "#62a63f" }}
            >
              <div
                style={{
                  width: "26px",
                  height: "26px",
                  WebkitMaskImage: `url(${process.env.NEXT_PUBLIC_BASE_PATH}/images/${state.selectedWidgetIcon || "widget-icon1"}.svg)`,
                  WebkitMaskRepeat: "no-repeat",
                  WebkitMaskPosition: "center",
                  WebkitMaskSize: "contain",
                  maskImage: `url(${process.env.NEXT_PUBLIC_BASE_PATH}/images/${state.selectedWidgetIcon || "widget-icon1"}.svg)`,
                  maskRepeat: "no-repeat",
                  maskPosition: "center",
                  maskSize: "contain",
                  backgroundColor: state.widgetIconColor || "#ffffff",
                }}
              />
            </div>
          </div>
        ...
        {(state.selectedLauncher === "label-only" ||
          ...
          >
            <div
              className="max-w-[180px] h-[60px] rounded-full flex items-center justify-center cursor-pointer px-5"
              style={{ backgroundColor: state.widgetBgColor || "#62a63f" }}
            >
